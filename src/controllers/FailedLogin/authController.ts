...
const verificationCode = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const saveCode = new VerificationCode({email, code: verificationCode})
    await saveCode.save();
...
const codeConfirm = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (getCodeUser.length === 0) return res.status(404).json('We do not have codes for verification')
    ...
    res.json({ message: 'Identity verification successful, proceed to reset your password' });
  } catch (error) {
    console.log(error)
  }
};
